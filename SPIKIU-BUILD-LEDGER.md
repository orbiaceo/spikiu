...
- ▶ **AKTIVER AUFTRAG (Teil 32) = „Lektion-Angebot ans Gesprächsende + Audio im Poster"** — Design fertig, Prototyp `prototyp-lektion-angebot-audio.html` GENEHMIGT, `AKTUELLER-AUFTRAG.md` bereit. 2 Dateien (`chat.html` Beenden/`offerLesson`/`makeLessonAndLeave`/Auflösung + `dashboard.html` Knöpfe raus/chronologisch/🔊-Poster via `audio.js`). `api/generate-lesson.js` + Poster-SCHEMA UNBERÜHRT; Generator-Request 1:1 aus `generateLessonFromLastConvo`. rolling-3. 🔊 nur Wortschatz+Dialog, nur Zielsprache. Offen-Notiz: Modus-Schluss-Replik darf nicht doppelt nach Lektion fragen (UI macht es) — am Gerät prüfen.
