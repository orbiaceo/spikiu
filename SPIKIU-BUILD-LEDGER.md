...
- ▶ **NÄCHSTES = AKTIVER AUFTRAG: Geführtes Gespräch P2 „Die Häppchen" (Vertrag B)** (`AKTUELLER-AUFTRAG.md`) — neuer Endpoint `api/haeppchen.js` + `haeppchen-modus.md` (Muster `lektor.js`, strikter JSON-Vertrag) + `vercel.json` (Function + includeFiles `*.md`) + `chat.html` Wortschatz- + Hörverständnis-Widgets (Stil Prototyp, inline, A/B grün/rot) + `audio.js`-Einbau (`speak`/`warm`, Fallback steckt drin) + Lade-Spinner i18n + Fortschrittsbalken (füllt bis Sprechen). Häppchen VOR dem P1-Rollenspiel einhängen; `gespraech.js`/Rollenspiel unberührt.
- ▶ **Danach (eigene Pakete):** (1) **P3 „Die Lektion"** — gentle close + `chat.html` schreibt `verlauf`→`lastConversation` + „als Lektion" real an `/api/generate-lesson` → Dashboard. (2) **Kleinkram-Paket 2 (claude.ai):** Genus-Begrüßung Dashboard (es „bienvenido"→„Te damos la bienvenida", invariabel; **Seele-Regel: in UI-Copy nie Genus-Kongruenz mit dem Nutzer**; de/en schon neutral) + Lesebegleiter-`intro`-Kürzung (`lesebegleiter.js`, de/es/en). (3) **Werkstatt-Inhalt Variante B** — 3–4 Schreibaufgaben nach `koennen`/`zielsprache`. (4) Paket B live klicken · A2 Tür-öffnen-Regel · Paket C UI-Sieb. Backlog: Audio Phase B/C (Gym-Einbau, Lektions-Vorrender) · Assessment-als-Gespräch · Legal-Sequenz · Gym-Raum · **Supabase + ElevenLabs = Phase 2**. Register tú/usted = Spikiu nach Kontext (Grundsatz 7), optionale Eine-Zeile.
