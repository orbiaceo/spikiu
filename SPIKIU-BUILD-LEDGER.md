...
- ▶ **NÄCHSTES = „Übungs-Varianten" (Punkt ①)** — Hörverständnis: richtige Option ZUFÄLLIG platzieren + neuer Typ „Wörter tippen" (Audio → gehörte Wörter markieren, Distraktoren=Minimalpaar). `api/haeppchen.js` (JSON-Vertrag um Übungstyp + Position) + `haeppchen-modus.md` + `chat.html` (Word-Spotting-Widget). Design genehmigt in `prototyp-feinschliff-iii.html`. Dann: **Kleinkram-Paket 2** (Genus-Begrüßung Dashboard es „bienvenido"→„Te damos la bienvenida"; **Seele-Regel: in UI-Copy nie Genus-Kongruenz**; de/en neutral · Lesebegleiter-`intro`-Kürzung) · **Audio überall I — Lesebegleiter** (Phonetik-Sermon kürzen + 🔊 + Mini-Transkription) · **Audio überall II — Reader** (🔊 an jeder Zielsprachen-Wendung; vorher `cap*-v2.html`-Struktur prüfen) · Werkstatt-Inhalt Variante B · Paket B live klicken · A2 Tür-öffnen-Regel · Paket C UI-Sieb. Backlog: Audio Phase B/C · Assessment-als-Gespräch · Legal-Sequenz · Gym-Raum · **Supabase + ElevenLabs = Phase 2**. Register tú/usted = Spikiu nach Kontext (Grundsatz 7).
