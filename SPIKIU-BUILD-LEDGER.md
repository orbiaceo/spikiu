...
- ▶ **NÄCHSTES = AKTIVER AUFTRAG: „Roleplay-Feinschliff"** (`AKTUELLER-AUFTRAG.md`, Teil 31) — `chat.html` (1: dauerhafter Ausstieg im Rollenspiel — Anderes Thema / Beenden→Lektion jederzeit erreichbar, Optik = `renderEndMenu`) + `gespraech-modus.md` (2a: keine Lob/„was willst du"-Blase am Szenenende, nur Figur-Schluss + `[SZENENENDE]` · 2b: zusammengehörige Sätze in EINER Blase, `---` nur für echte Gedankenwechsel) + `audio.js` (3: Frage-Intonation untersuchen — Text erreicht Piper mit ¿…?, vermutlich Modell-Grenze → ehrlich melden, ggf. Beta-Grenze, ElevenLabs Phase 2). P3-Szenenende feuert bereits. ✅
- ▶ **Danach: Kleinkram-Paket 2** (Genus-Begrüßung Dashboard es „bienvenido"→„Te damos la bienvenida"; **Seele-Regel: in UI-Copy nie Genus-Kongruenz**; de/en neutral · Lesebegleiter-`intro`-Kürzung) · **Audio überall I — Lesebegleiter** (Phonetik-Sermon kürzen + 🔊 + Mini-Transkription) · **Audio überall II — Reader** (🔊 an jeder Zielsprachen-Wendung; vorher `cap*-v2.html`-Struktur prüfen) · Werkstatt-Inhalt Variante B · Paket B live klicken · A2 Tür-öffnen-Regel · Paket C UI-Sieb. Backlog: Audio Phase B/C · Assessment-als-Gespräch · Legal-Sequenz · Gym-Raum · **Supabase + ElevenLabs = Phase 2**. Register tú/usted = Spikiu nach Kontext (Grundsatz 7). Dann Kleinkram-Paket 2.
