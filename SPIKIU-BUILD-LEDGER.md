...
- ▶ **NÄCHSTES (Bau bereit, aktiver Auftrag):** **Audio Phase A** (`AKTUELLER-AUFTRAG.md`, Spec `DESIGN-AUDIO-PIPER.md`) — auf Leos Wunsch (20.06.) VOR dem Kleinkram. `audio.js` (provider-agnostisch `speak(text,zielsprache)`) + Piper-Lib/WASM self-gehostet + 1 Beweis-Knopf; **vier Stimmen FINAL** (de `de_DE-thorsten-high` · es `es_ES-sharvard-medium` · en `en_US-lessac-high` · el `el_GR-rapunzelina-low`); Fallback Browser-Stimme.
