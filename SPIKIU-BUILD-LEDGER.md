...
- ▶ **NÄCHSTES = AKTIVER AUFTRAG: Geführtes Gespräch PAKET FEINSCHLIFF** (`AKTUELLER-AUFTRAG.md`, Teil 27) — 2 Dateien: `gespraech-modus.md` (Roleplay-Wand: in der Szene nie als Lehrer aus der Rolle, Figur reformuliert still, Korrektur erst am Ende/in der Lektion · `[[…]]`-Brücken-Vertrag) + `chat.html` (Spikiu-Zug → Sprechblase Zielsprache mit dezentem 🔊=`window.spkSpeak`, nur Zieltext · Übersetzung als gedämpftes Kästchen, kein Audio · User grüne Blase ohne 🔊 · Stil aus `prototyp-sprechblase-uebersetzung.html`). KEIN Endpoint, KEIN P3. Prototyp genehmigt.
