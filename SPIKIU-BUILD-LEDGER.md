...
- ▶ **NÄCHSTES (Bau bereit):** **Kleinkram-Paket** (AKTUELLER-AUFTRAG.md) — 3 Edits: Name-Leak `api/gespraech.js` Z.62 + Opener-Bullet `gespraech-modus.md` + Lesebegleiter-`intro` (de/es/en) in `lesebegleiter.js`. Gefunden 20.06. beim systematischen Testen.
- ▶ **Danach (eigene Pakete, der Reihe nach):** (1) **Lektion-Anschluss** — `chat.html` schreibt `lastConversation`, `schreibwerkstatt.html`-Knopf real an `/api/generate-lesson` (heute Attrappe); Test nur als echter Lerner. (2) **Werkstatt-Inhalt Variante B** — Auswahl 3–4 Schreibaufgaben, Niveau nach `koennen`, Sprache nach `zielsprache`. (3) Paket B live klicken · A2 Tür-öffnen-Regel · Paket C UI-Sieb. Backlog: Assessment-als-Gespräch · Legal-Sequenz · Gym · **Supabase = Phase 2** (Wiederkehr/Cross-Device/Tester-Sicht/Stripe).
