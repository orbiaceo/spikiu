...
- ▶ **NÄCHSTES = P3 „Die Lektion"** — (a) **Korrektur-Karte am Szenenende** (GENEHMIGT: zwei Spalten „Du hast gesagt → Besser", farbcodiert, NUR kommunikativ unpassend, OHNE Erklärung) + (b) gentle close + drei Türen (weiter üben / plaudern / **als Lektion**) + (c) `chat.html` schreibt `verlauf`→`lastConversation` + „als Lektion" real an `/api/generate-lesson` → Dashboard (tilgt den Lektion-Disconnect) + (d) die Lektion erklärt das WARUM. Dann: **Kleinkram-Paket 2 (claude.ai)** — Genus-Begrüßung Dashboard (es „bienvenido"→„Te damos la bienvenida"; **Seele-Regel: in UI-Copy nie Genus-Kongruenz**; de/en neutral) + Lesebegleiter-`intro`-Kürzung. Dann: **Audio überall I — Lesebegleiter** (Phonetik-Sermon kürzen + 🔊 + Mini-Transkription) · **Audio überall II — Reader** (🔊 an jeder Zielsprachen-Wendung; vorher `cap*-v2.html`-Struktur prüfen) · Werkstatt-Inhalt Variante B · Paket B live klicken · A2 Tür-öffnen-Regel · Paket C UI-Sieb. Backlog: Audio Phase B/C (Gym-Einbau, Lektions-Vorrender; el-Onset-Artefakt) · Assessment-als-Gespräch · Legal-Sequenz · Gym-Raum · **Supabase + ElevenLabs = Phase 2**. Register tú/usted = Spikiu nach Kontext (Grundsatz 7).
