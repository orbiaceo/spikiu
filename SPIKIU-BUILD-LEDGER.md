...
- ▶ **NÄCHSTES = AKTIVER AUFTRAG: „Antwort-Paletten + Audio-Vorwärmen"** (`AKTUELLER-AUFTRAG.md`, Teil 30) — **Teil A** `gespraech-modus.md` + `chat.html`: im geführten Thema-Rollenspiel 2–3 klickbare Antwort-Chips (`[OPTIONEN]`-Signal, Zielsprache/gelernter Wortschatz, primär; Textfeld sekundär), im freien Sprechen KEINE Chips. **Teil B** `chat.html` + `audio.js`: `warm` schon beim Raum-Eintritt (kein 🔊-Retard) + Anti-Desync-Guard (Generations-Zähler) + Pro-Knopf-Spinner. Beide Teile unabhängig testbar. Antwort-Paletten genehmigt in `prototyp-feinschliff-iii.html`. **Danach zuerst: P3-Szenenende am echten Modell verifizieren** (Menü/Korrektur-Karte/`[SZENENENDE]`/Lektion-Anschluss — Code liegt, Trigger prüfen; die Paletten führen den Dialog jetzt zum sauberen Abschluss).
