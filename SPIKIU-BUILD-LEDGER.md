...
- ▶ **NÄCHSTES (Bau bereit, der Reihe nach):** (1) **Geführtes Gespräch P2 „Die Häppchen"** — Design-Entscheid VOR Bau: Inhalts-Vertrag **A** Inline-Marker `[HAEPPCHEN]{json}` vs **B** eigener Endpoint `api/haeppchen.js` (Muster `lektor.js`; claude.ai-Empfehlung **B**); dann Wortschatz- + Hörverständnis-Widgets in `chat.html` + `audio.speak()` + Fortschrittsbalken. (2) **Geführtes Gespräch P3 „Die Lektion"** — gentle close + `chat.html` schreibt `verlauf`→`lastConversation` + „als Lektion" real an `/api/generate-lesson` → Dashboard (tilgt den Lektion-Disconnect). (3) **Lesebegleiter-`intro`-Kürzung** (Rest des Kleinkram-Pakets, `lesebegleiter.js`, de/es/en). (4) **Werkstatt-Inhalt Variante B** — Auswahl 3–4 Schreibaufgaben, Niveau nach `koennen`, Sprache nach `zielsprache`. (5) Paket B live klicken · A2 Tür-öffnen-Regel · Paket C UI-Sieb. Backlog: Audio Phase B/C (Gym-Einbau, Lektions-Vorrender) · Assessment-als-Gespräch · Legal-Sequenz · Gym-Raum · **Supabase + ElevenLabs = Phase 2**.
