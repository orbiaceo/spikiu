...
export default async function handler(req, res) {
  ...
  const { name, nativeLang, profile, topic, focusInterest } = req.body;

  if (!name || !nativeLang || !profile || !topic) {
...
4. Adapt vocabulary and examples to their interests (${focusInterest || topic})
...
The first character of your response MUST be { and the last character MUST be }.`;
  ...
  const userMessage = `Generate a lesson on this topic: "${topic}"
