...
export default async function handler(req, res) {
  ...
  // Prompt-Caching: BEIDE Phasen teilen denselben stabilen Praefix, damit
  // Phase 1 und Phase 2 denselben Cache treffen.
  const STABIL = docs.seele + '\n\n' + docs.taller;

  let system, userMsg, fallbackTokens;
  if (istPhase2) {
    system = [
      { type: 'text', text: STABIL, cache_control: { type: 'ephemeral' } },
      { type: 'text', text: laufzeitProfil(p, thema || null) + '\n' +
  ...
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        // Pruefen und Rueckmelden nach festem Schema — Haiku genuegt.
